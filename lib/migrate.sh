...
deploy_limine_to_esp() {
    ...
    # Each `|| return 1` matters: called via `|| {...}` in cmd_migrate,
    # which suspends errexit for this whole body, so a swallowed cp
    # failure could let GRUB be removed atop a missing Limine.
...
# Matches by loader path (e.g. \EFI\opensuse\grubx64.efi), not by NVRAM
# label - the label is whatever bootloader-id the original installer
# used, rarely "XeroLinux", so label matching silently found nothing.
# Matching by exact path also avoids a bare "any grubx64.efi" deleting an
# unrelated OS's GRUB in a dual-boot setup: only paths remove_grub
# actually found on this ESP (grub_efi_paths) are ever considered.
...
# Prune kernel/initramfs images in the ESP that Limine no longer
# references. Safety model: limine.conf is authoritative - only delete ESP
# files under a <machine-id>/ dir NOT referenced by a path:/module_path:
# line (the running kernel is always referenced, so it's never deletable).
# If no referenced path parses at all, delete nothing.
...
# leaves stale UKIs behind on kernel removal/rename. Safety: only deletes a
# UKI limine.conf doesn't reference, and only when it references at least
# one EFI/Linux UKI at all (otherwise the boot scheme is unknown, e.g.
# direct EFI-stub boot - delete nothing rather than risk a booted image).
...
# Re-runs Limine's own deployment/registration in place: reinstalls its
# packages, redeploys its EFI binaries, re-registers its NVRAM entry, and
# regenerates kernel entries. Never touches GRUB, and never rewrites
# limine.conf's chainload section (re-appending risks duplicates on an
# already-migrated system). The low-risk fix for "Limine seems broken" -
# it works identically with or without Secure Boot, so there's never a
# functional reason to need GRUB back.
cmd_repair_limine() {
    if [[ "$(detect_bootloader)" != "limine" ]]; then
        emit_event "error" "error" "Limine is not the active bootloader; nothing to repair."
        return 1
    fi
    local esp_mountpoint
    esp_mountpoint="$(find_esp_mountpoint)" || {
    ...
    emit_event "repair_step" "info" "Reinstalling Limine packages"
    install_limine_packages || {
        emit_event "error" "error" "Failed to reinstall Limine packages."
        return 1
    }

    local other_os skip_fallback=0
    other_os="$(detect_other_os)"
    has_non_windows_other_os "$other_os" && skip_fallback=1

    emit_event "repair_step" "info" "Redeploying Limine to the EFI system partition"
    deploy_limine_to_esp "$esp_mountpoint" "$skip_fallback" || {
        emit_event "error" "error" "Failed to redeploy Limine to the ESP."
        return 1
    }

    emit_event "repair_step" "info" "Re-registering Limine's EFI boot entry"
    ...
    register_efi_boot_entry "$esp_disk" "$esp_part_num" || {
        emit_event "error" "error" "Failed to re-register Limine's EFI boot entry."
        return 1
    }

    emit_event "repair_step" "info" "Regenerating initramfs"
    run_cmd /usr/bin/mkinitcpio -P || {
        emit_event "error" "error" "Failed to regenerate initramfs."
        return 1
    }

    emit_event "repair_step" "info" "Regenerating Limine boot entries"
    run_limine_mkinitcpio || {
        emit_event "error" "error" "Failed to regenerate Limine boot entries via limine-mkinitcpio."
        return 1
    }

    emit_event "repair_step" "info" "Verifying Limine's EFI boot entry, ESP deployment, and limine.conf kernel entry"
    if ! verify_limine_entry || ! verify_limine_deployed "$esp_mountpoint" || ! verify_limine_conf_has_kernel_entry "${esp_mountpoint}/limine.conf"; then
        emit_event "error" "error" "Limine's EFI boot entry, ESP deployment, or limine.conf kernel entry could not be verified after repair."
        return 1
    fi

    run_boot_doctor_checks || true

    # DRY_RUN-aware: this whole function also runs as a preview (the GUI
    # dialog starts with `repair-limine --dry-run` automatically before
    # the user clicks anything).
    if [[ "${DRY_RUN:-0}" == "1" ]]; then
        emit_event "repair_done" "info" "Clear to repair. Nothing has changed yet - press Apply Repair to complete the repair."
    else
        emit_event "repair_done" "info" "Limine repair complete."
    fi
}
